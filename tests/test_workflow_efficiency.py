...
class WorkflowEfficiencyTest(unittest.TestCase):
    def test_e0003_r3_r5_builder_chains_and_validation_ownership(self) -> None:
        ...
        self.assertIn("repair_limit_reached", normalized_ticket_build)
        ...
        self.assertIn("explicit `heavy` has two", normalized_economy)
    ...
    def test_execution_intensity_and_ticket_flow_plan_artifact_contract(self) -> None:
        ...
        self.assertIn("Additive/reversible schema migration", intensity)
        self.assertIn("add targeted data review, not the full heavy workflow", intensity)
        self.assertIn("One risk-focused native planner", intensity)
        self.assertIn("at most one merged specialist", intensity)
        self.assertNotIn("12 max", intensity)
        ...
        self.assertNotIn("ticket-run-budget-v1", ticket_flow)
        self.assertNotIn("run-budget <activation_key>", ticket_flow)
        self.assertIn("no cumulative model-session reservation", ticket_flow)
        self.assertIn("Session count must not stop", ticket_flow)
        ...
        self.assertNotIn("delivery ceiling = 2 * direct_steps", milestone)
        self.assertNotIn("bin/phase-contract epic-budget", milestone)
        self.assertNotIn("epic-run-budget-v1", epic)
        self.assertIn("no epic-wide model-session ledger", epic)
    ...
    def test_ticket_artifact_and_memory_hydration_is_delegated_off_parent(self) -> None:
        ...
        self.assertIn('There is no fixed', curator)
        self.assertIn('as long as necessary', curator)
        self.assertIn('Include a fact only when omitting it could change', curator)
        ...
        self.assertIn('packet_status: ready', curator)
        self.assertNotIn('packet_status: overflow', curator)
    ...
    def test_ticket_deploy_owns_the_complete_staging_repair_chain(self) -> None:
        ...
        self.assertIn("two only for explicit `heavy`", wrapper)
