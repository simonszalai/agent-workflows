...
- If no override: check status is `ready_to_deploy_production` (standalone ticket) or
  `ready_to_deploy_staging` (epic — staging is epic-only)
- If override provided: accept any non-terminal status (not `completed`/`abandoned`/`on_ice`)
