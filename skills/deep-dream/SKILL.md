...
description: >-
  ...
  the orchestrator and skeptic critics adjudicate. Memory mutations auto-apply after adversarial
  survival; skill/workflow edits are gated for approval, then committed.
user_invocable: true
argument-hint: "[project] [--all] [--since 14d] [--auto-skills] [--dry-run]"
max_turns: 400
---
...
/deep-dream --auto-skills       # Also auto-apply surviving skill/workflow edits (no gate)
...
skill propagates to every project and every machine (see the distribution model in
`agent-workflows/CLAUDE.md`). The skill channel earns a human gate by default.
...
2. **No mutation before scrutiny.** A candidate is applied only after it survives the full
   adversarial loop (Phase 3). Freshly-generated proposals are never executed directly.
...
5. **Never consolidate away a hard-won rule.** Starred entries and CLAUDE.md rules (e.g. "flow
   failures are intentional observability", "Atlas additive-only") are load-bearing. Treat them
   with extra suspicion before any merge/abstract/delete.
...
2. **Set the evidence window.** Load the state file (below). Window start =
   `max(last_run_high_water_mark, now − --since)` (default `--since 14d`). Record the window.
...
```json
{
  ...
  "high_water_mark": {"ts": "2026-06-28T00:00:00Z", "global": "..."}
}
```
Create `~/.local/share/autodev-deepdream/` on first run. Use the per-project high-water mark to
stay incremental; re-runs only ingest new sessions/tickets past the mark.
...
- **Tie findings to fixes, both directions.** If the evidence shows a failure that a *recent*
  ...
  entry describes a gotcha whose root cause was permanently fixed, that entry is now stale (M:
  DELETE/UPDATE).
...
delete entry X") execute the **skill side first** (gated) and the **memory side only after** the
skill edit is committed — never delete the source of truth before its replacement is durable.
...
**Date:** YYYY-MM-DD  **Project:** <project>  **Mode:** <full | dry-run | --auto-skills>
...
- MIGRATE entry "<title>" → skills/<name> (reusable methodology); entry deleted after commit
...
1. **Evidence first, code is truth.** Every candidate cites a concrete locator; the codebase
   arbitrates every claim. No locator → no candidate.
...
5. **Two tiers of autonomy.** Memory auto-applies after survival; shared-skill edits are gated
   and must be committed. Never blur the two.
...
8. **Idempotent & incremental.** The high-water mark means a second run the same day does almost
   nothing. Re-runs converge, they don't churn.
```
