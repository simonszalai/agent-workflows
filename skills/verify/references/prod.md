...
Spawn `verifier` agent(s) to verify different aspects simultaneously:
...
The verify command spawns `verifier` agents in parallel with different focus areas:

| Agent                 | Focus           | Purpose                              | Tools Used                              |
| --------------------- | --------------- | ------------------------------------ | --------------------------------------- |
| `verifier` | Prefect flows   | Check flow/task run states and logs  | Bash (Prefect CLI)                      |
| `verifier` | Service logs    | Search Render logs for evidence      | Render MCP                              |
| `verifier` | Database state  | Query database state and schema      | Production database MCP                 |
| `verifier` | Data quality    | Validate data correctness            | Production database MCP                 |
