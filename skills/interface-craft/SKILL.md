...
description: "The one UI/UX skill — a single, non-contradictory set of principles for building great interfaces in any project. Foundations (static visual fundamentals: hierarchy, whitespace, alignment, contrast, rhythm, proximity, balance, scale, unity, the professional-UI gotchas + pre-delivery checklist), Shipping Polished UI (10 craft rules: house easing/shadow/duration tokens, drag physics, snap points, blur entrances, layered shadows, tactile press, grid reveals, state-driven design), Storyboard Animation (human-readable animation DSL), DialKit (live control panels for tuning values), and Design Critique (systematic UI review, Josh Puckett's methodology). Use whenever code touches UI — design, build, review, polish, or critique. Triggers on: UI, UX, interface, component, layout, design, hierarchy, whitespace, spacing, alignment, contrast, typography, color, shadow, depth, accessibility, responsive, dark mode, animate, animation, transition, storyboard, entrance, motion, spring, easing, timing, tokens, design tokens, blur, drag, snap, momentum, tactile, press, haptics, reveal, FLIP, dialkit, sliders, controls, tune, tweak, critique, review, feedback, audit, improve, polish, refine, redesign."
