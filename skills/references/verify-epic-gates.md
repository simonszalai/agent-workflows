...
| staging | BLOCKED | write/update canonical staging gate `verification_evidence` plus per-step ticket artifacts for every included step, with blocker ground-truth evidence and the staging-autonomy repair packet on affected steps; update the epic summary; leave milestone and step statuses unchanged; return `staging_safe`/`owner_repair` packets to `/milestone-flow` |
