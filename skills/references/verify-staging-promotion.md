...
1. **Finalized contract:** the deployment guide was `FINALIZED` and every staging row was graded.
   A derived `PASS (contract-missing)` cannot auto-promote — except when the scope is classified
   `tiny_safe` (ticket-verify §2a) with every derived row passed on fresh post-activation data;
   record the tier classification evidence in the promotion report.
...
3. **Low-risk diff:** no schema/migration, deploy-config, auth/security, or payment path changed.
   Use the same path classification as `/ticket-promote` and `landing-policy.md`; reuse a
   `risk_tier` classification already recorded for the same diff SHA instead of re-deriving it.
