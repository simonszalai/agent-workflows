...
## Step 4: Ingest via compound store procedure

For each parsed file, use the **compound** skill's store procedure (see `compound/references/store-procedure.md`) to ingest. Load that skill and
...
as required by the compound skill's store procedure (each ingestion decision is informed by what
