...
Tickets and epics use **separate** status enums. A ticket can never hold an epic-only
status; shared value names are coincidental.

**`ticket_status` (11):** `backlog, planning, planned, building, merged,
ready_to_deploy_production, to_verify_prod, verify_prod_failed, completed, abandoned,
on_ice`

**`epic_status` (13):** `backlog, planning, planned, building, ready_to_deploy_staging,
to_verify_staging, verify_staging_failed, ready_to_deploy_production, to_verify_prod,
verify_prod_failed, completed, abandoned, on_ice`

The **staging segment** (`ready_to_deploy_staging → to_verify_staging →
verify_staging_failed`) is **epic-only** — standalone tickets never enter staging.

Three paths through the lifecycle:

- **Standalone ticket** — direct to main, batched prod verify:
  `backlog → [planning → planned] → building → ready_to_deploy_production → to_verify_prod
  → completed`
- **Epic-member ticket** — the epic carries it through staging/prod:
  `backlog → [planning] → building → merged` → (epic completes) → `completed`
- **Epic** — full staging gate then prod:
  `backlog → planning → planned → building → ready_to_deploy_staging → to_verify_staging →
  ready_to_deploy_production → to_verify_prod → completed`

`merged` (epic-member ticket) means the member's build is complete and its branch is pushed —
**not** that it is merged to main. The actual merge to main happens during the epic's ordered
prod promote (the deferred epic-walk), not at this status; `auto-build` reaches `merged` by
pushing only, and `auto-verify` deliberately does not merge an epic member on staging PASS.

A ticket is an **epic member** iff its `epic_id` is set. **Status is a readout the workflows
write; humans trigger workflows.** There is no `approved` status — approval is the act of
leaving `planned` (which triggers build-todo generation + build). There is no bare `active`
or `to_verify`. Skills set the specific statuses (`planning`, `building`, `merged`,
`ready_to_deploy_production`, `to_verify_prod`) as they progress; the staging statuses are
written on the **epic**, never the ticket.

### Epics

An **epic** is the ordered, cross-repo execution plan for one coherent feature within a
single project (never across projects). It holds ≥1 member ticket per involved repo, in a
defined order (a uuid DAG), and **carries the staging confidence gate**. Epics have display
IDs `E####`, their own `epic_status` lifecycle, and their own artifacts.

| Tool | Purpose |
|---|---|
| `create_epic` / `list_epics` / `get_epic` / `update_epic` / `delete_epic` | Epic CRUD |
| `assign_ticket_to_epic` / `unassign_ticket_to_epic` | Manage membership |
| `set_epic_member_deps` | Define member ordering (uuid DAG edges) |
| `create_epic_artifact` | Add plan/source artifacts to an epic |

`create_ticket` / `update_ticket` accept an `epic_id` to attach a ticket to an epic. The
automated epic *walk* (ordered cross-repo deploy + integrated staging verify + ordered prod
promote) is hand-orchestrated for now; the per-ticket skills run on each member.
...
2. **Plan** — `update_ticket(status="planning")`, then `create_artifact(artifact_type="plan", content=...)`, then `update_ticket(status="planned")`
3. **Build todos** — `create_artifact(artifact_type="build_todo", sequence=N, ...)`
4. **Build** — `update_ticket(status="building")`; `update_artifact(status="in_progress")` then `update_artifact(status="complete")`
5. **Review** — `create_artifact(artifact_type="review_todo", sequence=N, ...)`
6. **Resolve** — `update_artifact(status="resolved")` for each review finding
7. **Finish build** — `update_ticket(status="merged")` (epic member) or `update_ticket(status="ready_to_deploy_production")` (standalone)
8. **Verify** — `update_ticket(status="to_verify_prod")` after deploy
9. **Close** — `update_ticket(status="completed")`

### Autonomous Workflows

- `/auto-build`: Builds, reviews, resolves, pushes the branch for a `planned` ticket. Does
  NOT create a PR — that is the first action of `/auto-deploy`.
- `/lfg`: Autonomous end-to-end (no tickets) — research -> plan -> build -> review -> PR
- `/auto-flow`: Like `/lfg` but with full ticket lifecycle tracking (FNNN/BNNN/RNNN)
